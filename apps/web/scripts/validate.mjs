...
for (const marker of ["const isGuestMode=()=>sessionStorage.getItem('hl-guest-mode')==='1'","if(isGuestMode()){root.classList.add('hidden');return}","sessionStorage.removeItem('hl-guest-mode')","isGuestMode,refreshSession"]) if (!authModule.includes(marker)) throw new Error(`Missing explicit non-authenticated guest-mode boundary: ${marker}`);
...
console.log(`Validated HYDROLAND shell, six role selectors, ${jsFiles.length} JavaScript modules, ${cssFiles.length} style modules, branding, IDs, responsiveness, accessibility, guest/auth separation and pinned resilient MapLibre runtime markers.`);
