...
for (const marker of ["clearProtectedView","window.HydrolandProfileData=undefined","window.addEventListener('pageshow'","clearSession();clearProtectedView();","setTimeout(emitAuthChanged,0)","showLogin=message=>{clearProtectedView()"] ) if (!authModule.includes(marker)) throw new Error(`Missing browser/session closure marker: ${marker}`);
