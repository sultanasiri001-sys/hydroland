...
for (const marker of ["const isGuestMode=()=>sessionStorage.getItem('hl-guest-mode')==='1'","if(isGuestMode()){root.classList.add('hidden');return}","sessionStorage.removeItem('hl-guest-mode')","isGuestMode,refreshSession","if(!sessionStorage.getItem('hl-refresh-token'))throw new Error('AUTH_REQUIRED')"]) if (!authModule.includes(marker)) throw new Error(`Missing explicit anonymous/guest authentication boundary: ${marker}`);
