(()=>{
  ...
  function sourceMeta(log){
    ...
    return {tripId:marker[1],label:'من رحلة HYDROLAND'};
  }
  ...
  async function load(){
    ...
    try{
      ...
      list.innerHTML=logs.length?logs.map(log=>{const source=sourceMeta(log);return `<article class="hl-log-item"><div><b>${log.siteName}</b><small>${formatDate(log.diveDate)} · ${log.maxDepthM}م · ${log.durationMin} دقيقة</small>${source?`<small class="hl-log-source">${source.label} · ${source.tripId.slice(0,8)}</small>`:''}</div><span>${statusLabel(log.status)}</span></article>`}).join(''):'<small>لا توجد غوصات مسجلة بعد.</small>';
