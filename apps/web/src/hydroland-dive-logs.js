(()=>{
  ...
  function statusLabel(status){return status==='VERIFIED'?'موثق':status==='REJECTED'?'مرفوض':'مسودة';}
  ...
  async function load(){
    ...
    try{
      ...
      list.innerHTML=logs.length?logs.map(log=>`<article class="hl-log-item"><div><b>${log.siteName}</b><small>${formatDate(log.diveDate)} · ${log.maxDepthM}م · ${log.durationMin} دقيقة</small></div><span>${statusLabel(log.status)}</span></article>`).join(''):'<small>لا توجد غوصات مسجلة بعد.</small>';
