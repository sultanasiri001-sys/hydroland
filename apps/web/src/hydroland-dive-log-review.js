(()=>{
  ...
  async function load(){if(!token()){setState('يتطلب دخول إداري');return}try{setState('جارٍ التحميل');const r=await fetch(api()+'/dive-logs/admin/review',{headers:headers()});if(!r.ok)throw new Error();render(await r.json());setState('متصل بالخادم')}catch{setState('تعذر تحميل السجلات')}}
  async function decide(id,status){const reason=status==='REJECTED'?window.prompt('سبب الرفض - اختياري')||'':undefined;try{setState('جارٍ حفظ القرار');const r=await fetch(api()+'/dive-logs/admin/review/'+encodeURIComponent(id),{method:'PATCH',headers:headers(),body:JSON.stringify({status,reason})});if(!r.ok)throw new Error();await load();window.HydrolandDiveLogs?.reload?.();setState(status==='VERIFIED'?'تم توثيق السجل':'تم رفض السجل')}catch{setState('تعذر حفظ القرار')}}
