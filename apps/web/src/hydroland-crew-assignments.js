(()=>{
  ...
  function render(rows){
    const host=panel.querySelector('[data-crew-list]');host.textContent='';
    if(!rows.length){const p=document.createElement('p');p.textContent='لا توجد مهام رحلة حالياً.';host.appendChild(p);return;}
    rows.forEach(item=>{
      const card=document.createElement('article');card.className='hl-crew-assignment-card';
      const head=document.createElement('div');const title=document.createElement('strong');title.textContent=item.tripTitle||'رحلة HYDROLAND';const meta=document.createElement('small');meta.textContent=[item.role,item.startsAt?fmt(item.startsAt):'',item.endsAt?`حتى ${fmt(item.endsAt)}`:''].filter(Boolean).join(' · ');head.append(title,meta);
      const badge=document.createElement('span');badge.className='hl-status';badge.textContent=statusLabel(item.status);
      const actions=document.createElement('div');actions.className='hl-member-actions';
      if(item.status==='PENDING'){
        const accept=document.createElement('button');accept.type='button';accept.className='btn btn-primary';accept.textContent='قبول المهمة';accept.onclick=()=>respond(item.id,'ACCEPTED',accept);
        const reject=document.createElement('button');reject.type='button';reject.className='btn';reject.textContent='رفض المهمة';reject.onclick=()=>respond(item.id,'REJECTED',reject);
        actions.append(accept,reject);
      }
      card.append(head,badge,actions);host.appendChild(card);
    });
  }
  async function load(){
    if(!token()){state('يتطلب تسجيل الدخول');return}
  ...
  window.addEventListener('hydroland:booking-confirmed',()=>{if(!panel.hidden)load()});
  window.HydrolandCrewAssignments={reload:load};
})();