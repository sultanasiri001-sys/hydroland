(()=>{
  ...
  const ensureStyle=()=>{
    if(document.getElementById('hl-account-center-style'))return;
    const style=document.createElement('style');style.id='hl-account-center-style';style.textContent=`
      .hl-account-dialog{width:min(760px,calc(100vw - 24px));max-height:min(82vh,760px);border:1px solid rgba(120,191,224,.2);border-radius:24px;background:#071f31;color:#edf7fb;padding:0;box-shadow:0 28px 90px rgba(0,0,0,.45)}
      .hl-account-dialog::backdrop{background:rgba(1,10,18,.7);backdrop-filter:blur(5px)}
      .hl-account-head{display:flex;align-items:center;justify-content:space-between;padding:1rem 1.1rem;border-bottom:1px solid rgba(120,191,224,.14)}
      .hl-account-head h2{margin:.2rem 0 0}.hl-account-head button{border:0;background:transparent;color:#fff;font-size:1.6rem;cursor:pointer}
      .hl-account-body{padding:1rem;display:grid;gap:.8rem;overflow:auto;max-height:66vh}
      .hl-account-card{display:grid;gap:.55rem;padding:1rem;border:1px solid rgba(120,191,224,.14);border-radius:18px;background:rgba(255,255,255,.035)}
      .hl-account-card-head{display:flex;align-items:flex-start;justify-content:space-between;gap:.8rem}.hl-account-card h3{margin:0;font-size:1rem}.hl-account-card p,.hl-account-card small{margin:0;color:#9fbdcc}
      .hl-status{font-size:.72rem;font-weight:900;padding:.3rem .55rem;border-radius:999px;background:rgba(255,255,255,.08)}
      .hl-status.confirmed,.hl-status.active,.hl-status.read{color:#90e7ba;background:rgba(68,190,126,.12)}.hl-status.cancelled,.hl-status.rejected{color:#ffaaa0;background:rgba(255,100,90,.11)}.hl-status.pending{color:#f6d88d;background:rgba(229,180,95,.12)}
      .hl-account-actions{display:flex;gap:.55rem;flex-wrap:wrap}.hl-account-actions button{border:1px solid rgba(120,191,224,.18);border-radius:12px;padding:.58rem .8rem;background:rgba(255,255,255,.05);color:#eaf7fb;font-weight:800;cursor:pointer}.hl-account-actions button.danger{border-color:rgba(255,120,110,.28);color:#ffc0b9}.hl-account-actions button:disabled{opacity:.5;cursor:not-allowed}
      .hl-account-empty{text-align:center;padding:2rem 1rem;color:#9fbdcc}.hl-account-error{color:#ffc0b9;text-align:center;padding:1rem}
      .hl-notification.unread{border-color:rgba(229,180,95,.32);background:rgba(229,180,95,.05)}
      @media(max-width:640px){.hl-account-dialog{width:calc(100vw - 16px);border-radius:20px}.hl-account-card-head{align-items:flex-start;flex-direction:column}}
    `;document.head.appendChild(style);
  };
  const ensureDialog=(id,title,eyebrow)=>{
    let dialog=document.getElementById(id);if(dialog)return dialog;
    dialog=document.createElement('dialog');dialog.id=id;dialog.className='hl-account-dialog';
    dialog.innerHTML=`<div class="hl-account-head"><div><p class="eyebrow">${esc(eyebrow)}</p><h2>${esc(title)}</h2></div><button type="button" aria-label="إغلاق">×</button></div><div class="hl-account-body"></div>`;
    dialog.querySelector('.hl-account-head button').addEventListener('click',()=>dialog.close());
  ...
  const renderBookings=async()=>{
    ...
    ensureStyle();const dialog=ensureDialog('hl-bookings-dialog','حجوزاتي','MY BOOKINGS · حجوزاتي');const body=dialog.querySelector('.hl-account-body');body.innerHTML='<div class="hl-account-empty">جارٍ تحميل الحجوزات...</div>';dialog.showModal();
    try{
  ...
  const notificationSummary=payload=>{if(!payload||typeof payload!=='object')return '';const entries=Object.entries(payload).filter(([,value])=>['string','number','boolean'].includes(typeof value)).slice(0,4);return entries.map(([key,value])=>`${key}: ${value}`).join(' · ')};
  const renderNotifications=async()=>{
    if(!requireAuth())return;
    ensureStyle();const dialog=ensureDialog('hl-notifications-dialog','الإشعارات','NOTIFICATIONS · الإشعارات');const body=dialog.querySelector('.hl-account-body');body.innerHTML='<div class="hl-account-empty">جارٍ تحميل الإشعارات...</div>';dialog.showModal();
    try{
  ...
  window.HydrolandAccountCenter={openBookings:renderBookings,openNotifications:renderNotifications};
})();