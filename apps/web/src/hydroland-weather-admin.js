(()=>{
  ...
  function render(settings){
    ...
    const provider=panel.querySelector('[data-weather-provider]');if(provider)provider.textContent=settings.provider==='NOT_SELECTED'?'غير محدد':settings.provider;
  ...
  async function load(){
    ...
    try{state('جارٍ تحميل الإعدادات');const r=await fetch(api()+'/trips/admin/weather-gate',{headers:headers()});const body=await r.json();if(!r.ok)throw new Error();render(body);state('متصل بالخادم')}catch{state('تعذر تحميل الإعدادات')}
  }
  async function save(payload){
    try{state('جارٍ حفظ الإعداد');const r=await fetch(api()+'/trips/admin/weather-gate',{method:'PATCH',headers:headers(),body:JSON.stringify(payload)});const body=await r.json();if(!r.ok)throw new Error();render(body);window.dispatchEvent(new CustomEvent('hydroland:weather-gate-changed',{detail:body}));state('تم حفظ الإعداد')}catch{state('تعذر حفظ الإعداد')}
  }
