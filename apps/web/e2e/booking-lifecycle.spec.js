...
const installApi=async page=>{
  ...
  const price={pricePerSeatMinor:0,currency:'SAR',configured:true};
  const trip=()=>({id:'trip-1',title:'رحلة جزيرة سمر',type:'BOAT_DIVE',startsAt:new Date(Date.now()+86400000).toISOString(),endsAt:new Date(Date.now()+90000000).toISOString(),capacity:3,bookedSeats:0,remainingSeats:3,status:'OPEN',price,safety:{decision:'ALLOWED'},weather:{snapshot:{decision:'ALLOWED',waveHeightM:0.5},evaluation:{blocking:false,decision:'ALLOWED'}}});
  ...
  await page.route(/\/api\/v1\/trips\/trip-1\/bookings$/,async route=>{state.submittedSeats=route.request().postDataJSON()?.seats;return json(route,{id:'booking-1',tripId:'trip-1',accountId:'account-e2e',status:state.bookingStatus,seats:state.submittedSeats,price,participants:state.participants},201)});
...
test('multi-seat free booking, participant editing and self-cancel work from the browser',async({page})=>{
  ...
  await expect(page.locator('#booking-price')).toContainText('مجانية');
