...
test('registration remains unauthenticated until email verification',async({page})=>{
  await page.route('**/api/v1/auth/register',route=>json(route,{verificationRequired:true},201));
...
test('email verification link creates the first authenticated session',async({page})=>{
  ...
  await page.route('**/api/v1/auth/email-verification/confirm',async route=>{
    receivedToken=(await route.request().postDataJSON()).token;
    return json(route,{accessToken:'verified-access',refreshToken:'verified-refresh'});
  });
...
test('recovery controls request generic outbox actions without creating a session',async({page})=>{
  const calls=[];
  ...
  expect(calls).toEqual([
    ['reset',{email:'recovery@hydroland.test'}],
    ['verify',{email:'recovery@hydroland.test'}]
  ]);
  expect(await page.evaluate(()=>sessionStorage.getItem('hl-refresh-token'))).toBeNull();
});
