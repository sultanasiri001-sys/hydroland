...
test('authenticated user reports a safety incident and admin resolves it with evidence',async({page})=>{
  ...
  const review=panel.locator('[data-safety-incident-id="incident-e2e"]');await expect(review).toBeVisible();await review.locator('[data-safety-incident-decision="UNDER_REVIEW"]').click();await expect.poll(()=>state.decision?.status).toBe('UNDER_REVIEW');
