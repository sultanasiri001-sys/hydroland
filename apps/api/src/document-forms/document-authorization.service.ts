...
export type DocumentAction='TEMPLATE_CREATE'|'TEMPLATE_LIST'|'DOCUMENT_CREATE'|'DOCUMENT_READ'|'SUBMIT'|'APPROVE'|'SIGN'|'ARCHIVE';
const permissions:Record<OrganizationMemberRole,ReadonlySet<DocumentAction>>={
 OWNER:new Set(['TEMPLATE_CREATE','TEMPLATE_LIST','DOCUMENT_CREATE','DOCUMENT_READ','SUBMIT','APPROVE','SIGN','ARCHIVE']),
 ADMIN:new Set(['TEMPLATE_CREATE','TEMPLATE_LIST','DOCUMENT_CREATE','DOCUMENT_READ','SUBMIT','APPROVE','SIGN','ARCHIVE']),
 OPERATOR:new Set(['TEMPLATE_LIST','DOCUMENT_CREATE','DOCUMENT_READ','SUBMIT']),
 INSTRUCTOR:new Set(['TEMPLATE_LIST','DOCUMENT_CREATE','DOCUMENT_READ','SUBMIT']),
 STAFF:new Set(['TEMPLATE_LIST','DOCUMENT_CREATE','DOCUMENT_READ','SUBMIT']),
