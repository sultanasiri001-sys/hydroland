...
import { LocalDocumentAssetStorage } from './document-asset-storage.service';

@Injectable()
export class DocumentAssetService {
  constructor(private readonly db:DatabaseService,private readonly authz:DocumentAuthorizationService,private readonly storage:LocalDocumentAssetStorage){}
  ...
  async uploadLogo(accountId:string,organizationId:string,mimeType:string,bytes:Buffer){
    ...
    const storageKey=`organizations/${organizationId}/document-assets/${randomUUID()}.${ext}`;
    await this.storage.put(storageKey,bytes,mimeType);
    try{return await this.db.organizationDocumentAsset.create({data:{organizationId,kind:'LOGO',mimeType,byteSize:bytes.length,sha256,storageKey}});}
    catch(e){throw e;}
  }
