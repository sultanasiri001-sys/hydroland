...
export class MoyasarPaymentService {
  ...
  checkoutConfig(payment:{id:string;bookingId:string;amountMinor:number;currency:string}){
    ...
    return{
      ...
      description:`HYDROLAND booking ${payment.bookingId}`,
      publishableApiKey:publishableKey,
      callbackUrl:callback.toString(),
      methods:['creditcard'],
      supportedNetworks:['mada','visa','mastercard'],
      metadata:{hydroland_payment_id:payment.id,booking_id:payment.bookingId},
    };
  }

  async fetchPayment(providerPaymentId:string):Promise<MoyasarPayment>{
    const id=this.paymentId(providerPaymentId);
    this.integrations.requireOperational('PAYMENT_PSP',{allowSandbox:true});
    this.assertProvider();
    const response=await this.request(`/payments/${encodeURIComponent(id)}`,{method:'GET'});
    return this.parsePayment(response);
  }

  async refund(providerPaymentId:string):Promise<MoyasarPayment>{
    const id=this.paymentId(providerPaymentId);
    this.integrations.requireOperational('PAYMENT_PSP',{allowSandbox:true});
    this.assertProvider();
    const response=await this.request(`/payments/${encodeURIComponent(id)}/refund`,{method:'POST'});
    return this.parsePayment(response);
  }
  ...
  verifyWebhook(input:unknown){
    ...
    const configured=process.env.MOYASAR_WEBHOOK_SECRET?.trim();
    const received=typeof body.secret_token==='string'?body.secret_token:'';
  ...
  private async request(path:string,init:RequestInit){
    ...
    if(!secret)throw new ServiceUnavailableException('Moyasar secret key is not configured.');
    const base=process.env.MOYASAR_API_BASE_URL?.trim()||'https://api.moyasar.com/v1';
    let url:URL;
    try{url=new URL(`${base.replace(/\/$/,'')}${path}`);}catch{throw new ServiceUnavailableException('Moyasar API endpoint is invalid.');}
    let response:Response;
    try{
      response=await fetch(url,{...init,headers:{Authorization:`Basic ${Buffer.from(`${secret}:`).toString('base64')}`,Accept:'application/json','Content-Type':'application/json',...(init.headers||{})},signal:AbortSignal.timeout(8_000)});
    }catch{throw new ServiceUnavailableException('Moyasar request failed.');}
    let payload:unknown={};
    try{payload=await response.json();}catch{payload={};}
    if(!response.ok)throw new ServiceUnavailableException(`Moyasar request failed (${response.status}).`);
    return payload;
  }
  ...
  private parsePayment(input:unknown):MoyasarPayment{
    ...
    const statuses=new Set<MoyasarPaymentStatus>(['initiated','paid','authorized','captured','failed','refunded','voided','verified']);
