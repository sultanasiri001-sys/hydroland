...
export class DiveLogsService {
  ...
  list(accountId: string) {
    return this.db.diveLog.findMany({ where: { accountId }, orderBy: { diveDate: 'desc' } });
  }
  ...
  async create(accountId: string, input: CreateDiveLogInput) {
    ...
    return this.db.diveLog.create({ data: { accountId, siteName, regionCode: input.regionCode?.trim() || null, diveDate, maxDepthM: input.maxDepthM, durationMin: input.durationMin, buddyName: input.buddyName?.trim() || null, instructorName: input.instructorName?.trim() || null, notes: input.notes?.trim() || null } });
  }
}
