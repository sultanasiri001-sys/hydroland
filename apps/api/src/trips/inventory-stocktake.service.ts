...
export class InventoryStocktakeService{
  ...
  async summary(id:string){
    ...
    const scannedIds=new Set(scans.map(row=>row.resourceId).filter((value:string|null):value is string=>Boolean(value))),missing=expected.filter(row=>!scannedIds.has(row.resourceId));
    const halala=(value:string|null|undefined)=>value?Number(value):0,expectedValueHalala=expected.reduce((sum,row)=>sum+halala(row.acquisitionCostHalala),0),missingValueHalala=missing.reduce((sum,row)=>sum+halala(row.acquisitionCostHalala),0),scannedValueHalala=scans.filter(row=>row.resourceId).reduce((sum,row)=>sum+halala(row.acquisitionCostHalala),0);
    const counts={matched:scans.filter(row=>row.result==='MATCHED').length,wrongLocation:scans.filter(row=>row.result==='WRONG_LOCATION').length,unregistered:scans.filter(row=>row.result==='UNREGISTERED').length,missing:missing.length,totalExpected:expected.length,totalScanned:scans.length,expectedValueHalala,scannedValueHalala,missingValueHalala};
