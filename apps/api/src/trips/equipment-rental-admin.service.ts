...
export class EquipmentRentalAdminService{
  ...
  async handoverScan(actorAccountId:string,id:string,input:{code?:string}){
    ...
    const refreshed=await this.db.$queryRaw<HandoverItem[]>`SELECT "id","resourceId","assetCodeSnapshot","equipmentNameSnapshot","handedOverAt","returnedAt" FROM "EquipmentRentalItem" WHERE "rentalId"=${id} ORDER BY "createdAt"`;const handedOverCount=refreshed.filter((row:HandoverItem)=>row.handedOverAt).length;
