...
export class CrewAssignmentService{
  ...
  private async qualificationPolicy():Promise<QualificationPolicy>{
    const [master,instructorRole,instructorCredential,captainRole,captainLicense,crewRole]=await Promise.all([
      this.policies.state('CREW','QUALIFICATION'),this.policies.state('INSTRUCTOR','ACTIVE_ROLE'),this.policies.state('INSTRUCTOR','VERIFIED_CREDENTIAL'),this.policies.state('CAPTAIN','ACTIVE_ROLE'),this.policies.state('CAPTAIN','VERIFIED_LICENSE'),this.policies.state('CREW','ACTIVE_ROLE'),
    ]);
    return {master,instructorRole,instructorCredential,captainRole,captainLicense,crewRole};
  }
  ...
  private async eligibleResources(tripId:string,excludeResourceId?:string,excludeAccountId?:string){
    ...
    const rows=await this.db.$queryRaw<CrewResourceRow[]>`
      ...
      FROM "CalendarResource" r JOIN "Account" ac ON ac."id"=r."referenceId"
  ...
  async dispatchForConfirmedBooking(tripId:string,bookingId:string){
    ...
    const qualified=await this.eligibleResources(tripId);
    const allocated=await this.db.$queryRaw<CrewResourceRow[]>`SELECT r."id",r."type",r."name",r."referenceId" FROM "CalendarAllocation" a JOIN "CalendarResource" r ON r."id"=a."resourceId" WHERE a."tripId"=${tripId} AND a."status"='ACTIVE' AND r."id"=ANY(${qualified.rows.map(r=>r.id)}::text[])`;
    for(const resource of allocated){const existing=await this.db.$queryRaw<AssignmentRow[]>`SELECT * FROM "CrewAssignment" WHERE "tripId"=${tripId} AND "resourceId"=${resource.id} AND "status" IN ('PENDING','ACCEPTED') ORDER BY "createdAt" DESC LIMIT 1`;if(!existing.length){const created=await this.db.$queryRaw<AssignmentRow[]>`INSERT INTO "CrewAssignment"("id","tripId","resourceId","accountId","roleType","status","createdAt","updatedAt") VALUES(gen_random_uuid()::text,${tripId},${resource.id},${resource.referenceId},${resource.type},'PENDING',NOW(),NOW()) RETURNING *`;await this.notify(resource.referenceId,'TRIP_CREW_ASSIGNMENT',{assignmentId:created[0].id,tripId,bookingId,tripTitle:trip.title,startsAt:trip.startsAt,endsAt:trip.endsAt,roleType:resource.type,resourceName:resource.name,actionRequired:true,policyReview:qualified.reviews});}else await this.notify(existing[0].accountId,'TRIP_BOOKING_CONFIRMED',{assignmentId:existing[0].id,tripId,bookingId,tripTitle:trip.title,startsAt:trip.startsAt,endsAt:trip.endsAt,roleType:existing[0].roleType,actionRequired:existing[0].status==='PENDING',policyReview:qualified.reviews});}
  ...
  mine(accountId:string){return this.db.$queryRaw<AssignmentWithTripRow[]>`SELECT c.*,t."title" AS "tripTitle",t."type" AS "tripType",t."startsAt",t."endsAt" FROM "CrewAssignment" c JOIN "Trip" t ON t."id"=c."tripId" WHERE c."accountId"=${accountId} ORDER BY t."startsAt" ASC,c."createdAt" DESC LIMIT 100`;}
  ...
  async respond(accountId:string,assignmentId:string,response:'ACCEPTED'|'REJECTED'){const rows=await this.db.$queryRaw<AssignmentRow[]>`SELECT * FROM "CrewAssignment" WHERE "id"=${assignmentId} LIMIT 1`;const assignment=rows[0];if(!assignment||assignment.accountId!==accountId)throw new NotFoundException('Crew assignment not found.');if(assignment.status!=='PENDING')throw new ConflictException('Crew assignment already answered.');if(response==='ACCEPTED'){await this.db.$executeRaw`UPDATE "CrewAssignment" SET "status"='ACCEPTED',"respondedAt"=NOW(),"updatedAt"=NOW() WHERE "id"=${assignmentId} AND "status"='PENDING'`;await this.notifyAdmins('CREW_ASSIGNMENT_ACCEPTED',{assignmentId,tripId:assignment.tripId,accountId});return{assignmentId,status:'ACCEPTED'};}await this.db.$executeRaw`UPDATE "CrewAssignment" SET "status"='REJECTED',"respondedAt"=NOW(),"updatedAt"=NOW() WHERE "id"=${assignmentId} AND "status"='PENDING'`;const replacement=await this.findReplacement(assignment);if(!replacement){await this.notifyAdmins('CREW_REPLACEMENT_REQUIRED',{assignmentId,tripId:assignment.tripId,roleType:assignment.roleType,rejectedByAccountId:accountId});return{assignmentId,status:'REJECTED',replacement:null,requiresAdminAction:true};}const replacementAssignment=await this.replaceAssignment(assignment,replacement);if(!replacementAssignment)throw new ConflictException('Crew replacement could not be created.');return{assignmentId,status:'REJECTED',replacement:{assignmentId:replacementAssignment.id,accountId:replacement.referenceId,resourceId:replacement.id,resourceName:replacement.name}};}

  private async findReplacement(assignment:AssignmentRow):Promise<CrewResourceRow|null>{const trip=await this.db.trip.findUnique({where:{id:assignment.tripId}});if(!trip)return null;const qualified=await this.eligibleResources(assignment.tripId,assignment.resourceId,assignment.accountId);const ids=qualified.rows.filter(r=>r.type===assignment.roleType).map(r=>r.id);if(!ids.length)return null;const candidates=await this.db.$queryRaw<CrewResourceRow[]>`SELECT r."id",r."type",r."name",r."referenceId" FROM "CalendarResource" r WHERE r."id"=ANY(${ids}::text[]) AND NOT EXISTS(SELECT 1 FROM "CalendarAllocation" a WHERE a."resourceId"=r."id" AND a."status"='ACTIVE' AND a."startsAt"<${trip.endsAt} AND a."endsAt">${trip.startsAt} AND a."tripId"<>${assignment.tripId}) AND NOT EXISTS(SELECT 1 FROM "CrewAssignment" c WHERE c."tripId"=${assignment.tripId} AND c."accountId"=r."referenceId" AND c."status" IN ('PENDING','ACCEPTED')) ORDER BY r."name" ASC LIMIT 1`;return candidates[0]??null;}

  private async replaceAssignment(assignment:AssignmentRow,replacement:CrewResourceRow,previousStatus?:'REASSIGNED'):Promise<AssignmentRow|null>{const created=await this.db.$transaction(async(tx:Prisma.TransactionClient)=>{if(previousStatus){const changed=await tx.$executeRaw`UPDATE "CrewAssignment" SET "status"=${previousStatus},"respondedAt"=NOW(),"updatedAt"=NOW() WHERE "id"=${assignment.id} AND "status"='PENDING'`;if(!changed)return null;}await tx.$executeRaw`UPDATE "CalendarAllocation" SET "resourceId"=${replacement.id},"updatedAt"=NOW() WHERE "tripId"=${assignment.tripId} AND "resourceId"=${assignment.resourceId} AND "status"='ACTIVE'`;const rows=await tx.$queryRaw<AssignmentRow[]>`INSERT INTO "CrewAssignment"("id","tripId","resourceId","accountId","roleType","status","replacesAssignmentId","createdAt","updatedAt") VALUES(gen_random_uuid()::text,${assignment.tripId},${replacement.id},${replacement.referenceId},${assignment.roleType},'PENDING',${assignment.id},NOW(),NOW()) RETURNING *`;return rows[0]??null;});if(!created)return null;const trip=await this.db.trip.findUnique({where:{id:assignment.tripId}});await this.notify(replacement.referenceId,'TRIP_CREW_ASSIGNMENT',{assignmentId:created.id,tripId:assignment.tripId,tripTitle:trip?.title??'HYDROLAND Trip',startsAt:trip?.startsAt??null,endsAt:trip?.endsAt??null,roleType:assignment.roleType,resourceName:replacement.name,replacementForAssignmentId:assignment.id,actionRequired:true});await this.notifyAdmins('CREW_REASSIGNED',{tripId:assignment.tripId,rejectedAssignmentId:assignment.id,replacementAssignmentId:created.id,replacementAccountId:replacement.referenceId,roleType:assignment.roleType});return created;}
}
