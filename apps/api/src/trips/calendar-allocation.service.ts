...
export type OperationalReadiness={status:'READY'|'REVIEW_REQUIRED'|'NOT_READY';checks:{crew:boolean;safety:boolean;weather:boolean;resources:boolean;boat:boolean;compliance:boolean};blockers:string[];missingResourceTypes:string[];policyReview?:{required:boolean;issues:string[];states:Record<string,string>}};
...
export class CalendarAllocationService{
  ...
  async calendar(fromRaw:string,toRaw:string){
    ...
    return Promise.all(trips.map(async(trip:CalendarTripRow)=>{
      ...
      const required=this.requiredResourceTypes(trip.type),allocatedTypes=new Set(tripResources.map((r:AllocationWithResourceRow)=>r.resourceType)),missing=required.filter((type:string)=>!allocatedTypes.has(type)),boatIds=tripResources.filter((r:AllocationWithResourceRow)=>r.resourceType==='BOAT').map((r:AllocationWithResourceRow)=>r.resourceId),boatRequired=trip.type.toUpperCase().includes('BOAT'),boatCompliance=boatRequired?await this.boats.evaluate(boatIds,bookedSeats):{ready:true,blocked:false,reviewRequired:false,issues:[] as string[],states:{}};
      const tripCompliance=await this.compliance.get(trip.id),complianceReview=tripCompliance.policyReview;
      const raw={crew:crewSummary.ready,safety:latestSafety?.decision==='ALLOWED',weather:weather.decision==='ALLOWED'||(!gate.enabled&&weather.decision!=='DEFERRED'),resources:required.length===0||missing.length===0,boat:!boatCompliance.blocked,compliance:!complianceReview.blocked};
      const issues:string[]=[...(boatCompliance.reviewRequired?boatCompliance.issues:[]),...(complianceReview.required?complianceReview.issues:[])];
      ...
      const checks={crew:apply('crew',raw.crew,crewPolicy),safety:apply('safety',raw.safety,safetyPolicy),weather:apply('weather',raw.weather,weatherPolicy),resources:apply('resources',raw.resources,resourcePolicy),boat:raw.boat,compliance:raw.compliance};
      ...
      const readiness:OperationalReadiness={status:readinessStatus,checks,blockers,missingResourceTypes:missing,policyReview:{required:issues.length>0,issues:[...new Set(issues)],states:{crew:crewPolicy.state,safety:safetyPolicy.state,weather:weatherPolicy.state,resources:resourcePolicy.state,...boatCompliance.states,complianceRegulatory:complianceReview.states.regulatory,compliancePermit:complianceReview.states.permit}}};
      return{id:trip.id,title:trip.title,type:trip.type,startsAt:trip.startsAt,endsAt:trip.endsAt,capacity:trip.capacity,status:trip.status,bookedSeats,remainingSeats:Math.max(0,trip.capacity-bookedSeats),safety:latestSafety,weather:{snapshot,gate,evaluation:weather},boatCompliance,tripCompliance,readiness,crew:{summary:crewSummary,members:tripCrew.map((m:CrewReadinessRow)=>({assignmentId:m.id,accountId:m.accountId,resourceId:m.resourceId,roleType:m.roleType,status:m.status,name:[m.firstName,m.lastName].filter(Boolean).join(' ')||m.resourceName,resourceName:m.resourceName}))},resources:tripResources.map((a:AllocationWithResourceRow)=>({id:a.id,resourceId:a.resourceId,startsAt:a.startsAt,endsAt:a.endsAt,status:a.status,resource:{type:a.resourceType,name:a.resourceName}}))};
    }));
  }
