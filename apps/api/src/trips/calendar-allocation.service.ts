...
export type OperationalReadiness = { status:'READY'|'REVIEW_REQUIRED'|'NOT_READY'; checks:{crew:boolean;safety:boolean;weather:boolean;resources:boolean}; blockers:string[]; missingResourceTypes:string[] };
...
export class CalendarAllocationService {
  ...
  async readinessForTrip(tripId:string):Promise<OperationalReadiness>{
    ...
    const from=new Date(trip.startsAt.getTime()-60000),to=new Date(trip.endsAt.getTime()+60000);const rows=await this.calendar(from.toISOString(),to.toISOString());const row=rows.find(item=>item.id===tripId);if(!row)throw new NotFoundException('Trip readiness unavailable.');return row.readiness as OperationalReadiness;
  }
  ...
  async calendar(fromRaw:string,toRaw:string){
    ...
    return trips.map((trip:CalendarTripRow)=>{
      const bookedSeats=trip.bookings.reduce((sum:number,b:{seats:number})=>sum+b.seats,0);
