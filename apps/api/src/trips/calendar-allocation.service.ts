...
export class CalendarAllocationService{
  ...
  async calendar(fromRaw:string,toRaw:string){
    ...
    return Promise.all(trips.map(async(trip:CalendarTripRow)=>{
      ...
      const checks={crew:apply('crew',raw.crew,crewPolicy),safety:apply('safety',raw.safety,safetyPolicy),weather:apply('weather',raw.weather,weatherPolicy),resources:apply('resources',raw.resources,resourcePolicy),boat:raw.boat,marine:raw.marine,equipment:raw.equipment,compliance:raw.compliance};
