...
type TripListRow = {
  ...
  safetyChecklists: Array<{ id:string; decision:string; items:unknown; notes:string|null; decidedAt:Date|null; createdAt:Date }>;
  [key:string]: unknown;
};

@Injectable()
export class TripsService {
  constructor(private readonly db:DatabaseService,private readonly weatherGate:WeatherGateService,private readonly participants:BookingParticipantService) {}
  ...
  async list(){
    ...
    const trips=(await this.db.trip.findMany({where:{status:'OPEN'},orderBy:{startsAt:'asc'},include:{bookings:{where:{status:{in:['PENDING','CONFIRMED']}},select:{seats:true}},safetyChecklists:{orderBy:{createdAt:'desc'},take:1,select:{id:true,decision:true,items:true,notes:true,decidedAt:true,createdAt:true}}}})) as TripListRow[];
  ...
  async book(accountId:string,tripId:string,seats:number){
    ...
    const booking=await this.db.serializable(async tx=>{
      ...
      const used=await tx.booking.aggregate({where:{tripId,status:{in:['PENDING','CONFIRMED']}},_sum:{seats:true}});if((used._sum.seats??0)+seats>trip.capacity)throw new ConflictException('Trip capacity reached.');
    ...
    return {...booking,participants:participantRows};
  }
