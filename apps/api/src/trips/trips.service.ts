...
export class TripsService {
  ...
  async book(accountId:string,tripId:string,seats:number){
    ...
    const result=await this.db.serializable(async tx=>{
      ...
      const weatherRows=await tx.$queryRaw<WeatherReviewRow[]>`SELECT "status","snapshot","forecastAt","fetchedAt" FROM "TripWeatherReview" WHERE "tripId"=${tripId} ORDER BY "fetchedAt" DESC,"createdAt" DESC LIMIT 1`,weatherReview=weatherRows[0]??null,weather=this.weatherGate.evaluateReview(weatherReview?.snapshot,weatherReview?.status,gateSettings);if(weather.blocking){if(weatherPolicy.enforce)throw new ConflictException(weather.reason||'Trip is unavailable because weather review is not approved.');if(weatherPolicy.review)reviewIssues.push('WEATHER_GATE');}
