...
export class EquipmentInventoryService{
  ...
  async get(resourceId:string){
    ...
    const [movements,inspection]=await Promise.all([this.history(resourceId),this.inspections.evaluate([resourceId])]);return{...rows[0],movements,inspection};
  }
  ...
  async assignCode(actorAccountId:string,resourceId:string,input:{assetCode?:string;barcodeValue?:string;qrValue?:string;serialNumber?:string|null;sku?:string|null;location?:string|null}){
    ...
    const duplicates=await this.db.$queryRaw<Array<{resourceId:string}>>`SELECT "resourceId" FROM "EquipmentBarcode" WHERE "resourceId"<>${resourceId} AND ("assetCode"=${assetCode} OR "barcodeValue"=${barcodeValue} OR "qrValue"=${qrValue} OR (${serialNumber}::text IS NOT NULL AND "serialNumber"=${serialNumber})) LIMIT 1`;
    if(duplicates.length)throw new ConflictException('Equipment asset, barcode, QR or serial number already exists.');
  ...
  async move(actorAccountId:string,resourceId:string,input:{movementType?:MovementType;toLocation?:string|null;tripId?:string|null;assignedAccountId?:string|null;notes?:string|null}){
    ...
    if(current.stockStatus==='RETIRED')throw new ConflictException('Retired equipment cannot return to circulation.');
    if(movementType==='CHECK_OUT'&&current.stockStatus!=='AVAILABLE')throw new ConflictException('Only available equipment can be checked out.');
    if(movementType==='CHECK_IN'&&current.stockStatus!=='CHECKED_OUT')throw new ConflictException('Only checked-out equipment can be checked in.');
    if(movementType==='RELEASE'&&!['MAINTENANCE','QUARANTINED'].includes(current.stockStatus))throw new ConflictException('Only maintained or quarantined equipment can be released.');
    ...
    if(tripId){const trip=await this.db.trip.findUnique({where:{id:tripId},select:{id:true,status:true,startsAt:true}});if(!trip)throw new NotFoundException('Trip not found.');if(['CANCELLED','COMPLETED'].includes(trip.status))throw new ConflictException('Equipment cannot be assigned to a closed trip.');}
    ...
    const result=await this.db.serializable(async tx=>{
      ...
      if(latest.stockStatus!==current.stockStatus)throw new ConflictException('Equipment inventory state changed. Refresh and retry.');
