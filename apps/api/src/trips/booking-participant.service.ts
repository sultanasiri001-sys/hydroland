...
export class BookingParticipantService {
  ...
  async ensureForBooking(bookingId:string,accountId:string,seats:number,client?:Prisma.TransactionClient){
    ...
    const current=await db.$queryRaw<ParticipantRow[]>`SELECT * FROM "BookingParticipant" WHERE "bookingId"=${bookingId}::uuid ORDER BY "createdAt" ASC`;if(current.length>=seats)return current;
    ...
    const profileRows=await db.$queryRaw<Array<{medicalFitnessStatus:string|null;medicalClearanceExpiresAt:Date|null}>>`SELECT "medicalFitnessStatus","medicalClearanceExpiresAt" FROM "DiverProfile" WHERE "accountId"=${accountId}::uuid LIMIT 1`;
    ...
    for(let index=current.length;index<seats;index++){
      if(index===0)await db.$executeRaw`INSERT INTO "BookingParticipant"("id","bookingId","accountId","fullName","certificationTitle","certificationNumber","certificationIssuer","eligibilityStatus","createdAt","updatedAt") VALUES(gen_random_uuid()::text,${bookingId}::uuid,${accountId}::uuid,${`${account.person.firstName} ${account.person.lastName}`.trim()},${credential?.title??null},${credential?.credentialNumber??null},${credential?.issuer??null},${primaryEligible?'ELIGIBLE':'PENDING'},NOW(),NOW())`;
      else await db.$executeRaw`INSERT INTO "BookingParticipant"("id","bookingId","accountId","fullName","eligibilityStatus","createdAt","updatedAt") VALUES(gen_random_uuid()::text,${bookingId}::uuid,NULL,${`مشارك ${index+1} - البيانات غير مكتملة`},'PENDING',NOW(),NOW())`;
    }
    return db.$queryRaw<ParticipantRow[]>`SELECT * FROM "BookingParticipant" WHERE "bookingId"=${bookingId}::uuid ORDER BY "createdAt" ASC`;
  }

  async listForOwner(accountId:string,bookingId:string){const booking=await this.db.booking.findUnique({where:{id:bookingId},select:{accountId:true}});if(!booking||booking.accountId!==accountId)throw new NotFoundException('Booking not found.');return this.db.$queryRaw<ParticipantRow[]>`SELECT * FROM "BookingParticipant" WHERE "bookingId"=${bookingId}::uuid ORDER BY "createdAt" ASC`;}
  ...
  async updateForOwner(accountId:string,bookingId:string,participantId:string,input:ParticipantInput){
    ...
    const rows=await this.db.$queryRaw<ParticipantRow[]>`SELECT * FROM "BookingParticipant" WHERE "id"=${participantId} AND "bookingId"=${bookingId}::uuid LIMIT 1`;const current=rows[0];if(!current)throw new NotFoundException('Participant not found.');
  ...
  async assertConfirmable(bookingId:string,seats:number){
    ...
    const rows=await this.db.$queryRaw<ParticipantRow[]>`SELECT * FROM "BookingParticipant" WHERE "bookingId"=${bookingId}::uuid`;
    const issues:string[]=[];
