...
export class TripCompletionService {
  ...
  async complete(reviewerAccountId:string,tripId:string,input:CompleteTripInput){
    ...
    const result=await this.db.$transaction(async(tx:Prisma.TransactionClient)=>{
      ...
      if(trip.status==='COMPLETED'){const existing=await tx.diveLog.count({where:{notes:{contains:`HYDROLAND_TRIP:${tripId}`}}});const seatCount=await tx.booking.aggregate({where:{tripId,status:'CONFIRMED'},_sum:{seats:true}});return{trip,createdDiveLogs:0,existingDiveLogs:existing,confirmedAccounts:existing,confirmedSeats:seatCount._sum.seats??0,alreadyCompleted:true};}
      ...
      const bookings=await tx.booking.findMany({where:{tripId,status:'CONFIRMED'},select:{accountId:true,seats:true}});if(!bookings.length)throw new ConflictException('Trip has no confirmed participants.');
      ...
      for(const booking of bookings){const id=randomUUID(),noteText=[marker,input.notes?.trim()].filter(Boolean).join(' | ');const inserted=await tx.$executeRawUnsafe(`INSERT INTO "DiveLog" ("id","accountId","siteName","regionCode","diveDate","maxDepthM","durationMin","instructorName","notes","status","createdAt","updatedAt","sourceTripId") VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,'DRAFT',NOW(),NOW(),$10) ON CONFLICT ("accountId","sourceTripId") DO NOTHING`,id,booking.accountId,siteName,input.regionCode?.trim()||null,trip.startsAt,Number(input.maxDepthM),Number(input.durationMin),input.instructorName?.trim()||null,noteText,tripId);createdDiveLogs+=inserted;}
      const completedTrip=await tx.trip.update({where:{id:tripId},data:{status:'COMPLETED'}});return{trip:completedTrip,createdDiveLogs,existingDiveLogs:bookings.length-createdDiveLogs,confirmedAccounts:bookings.length,confirmedSeats,alreadyCompleted:false};
    });
    await this.audit.record({action:'TRIP_COMPLETED',resource:'Trip',resourceId:tripId,metadata:{reviewerAccountId,operationalClearanceEventId:operational.clearance.id,readiness:operational.readiness,confirmedAccounts:result.confirmedAccounts,confirmedSeats:result.confirmedSeats,createdDiveLogs:result.createdDiveLogs,existingDiveLogs:result.existingDiveLogs}});
