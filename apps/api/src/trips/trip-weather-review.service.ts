...
export class TripWeatherReviewService{
  ...
  async refresh(actorAccountId:string,tripId:string){
    ...
    if(previous&&previous.snapshotHash===snapshotHash){
      ...
      const rows=await this.db.$queryRaw<TripWeatherReview[]>`INSERT INTO "TripWeatherReview"("tripId","provider","forecastAt","fetchedAt","snapshot","snapshotHash","status","createdAt","updatedAt") SELECT t."id",${snapshot.provider??'STORMGLASS'},${forecastAt},NOW(),${JSON.stringify(snapshot)}::jsonb,${snapshotHash},'PENDING',NOW(),NOW() FROM "Trip" t WHERE t."id"::text=${tripId} RETURNING *`;review=rows[0];
    }
  ...
  async decide(actorAccountId:string,tripId:string,input:{status?:TripWeatherReviewStatus;notes?:string|null}){
    ...
    const rows=await this.db.$queryRaw<TripWeatherReview[]>`UPDATE "TripWeatherReview" SET "status"=${status},"notes"=${notes},"reviewedByAccountId"=(SELECT a."id" FROM "Account" a WHERE a."id"::text=${actorAccountId} LIMIT 1),"reviewedAt"=NOW(),"updatedAt"=NOW() WHERE "id"::text=${current.id} RETURNING *`;
