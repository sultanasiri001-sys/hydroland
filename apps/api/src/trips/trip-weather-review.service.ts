...
export class TripWeatherReviewService{
  ...
  async location(tripId:string){const rows=await this.db.$queryRaw<TripLocation[]>`SELECT * FROM "TripOperationalLocation" WHERE "tripId"=${tripId} LIMIT 1`;return rows[0]??null;}
  ...
  async setLocation(actorAccountId:string,tripId:string,input:LocationInput){
    ...
    const rows=await this.db.$queryRaw<TripLocation[]>`INSERT INTO "TripOperationalLocation"("tripId","locationName","latitude","longitude","createdAt","updatedAt") VALUES(${tripId},${value.locationName},${value.latitude},${value.longitude},NOW(),NOW()) ON CONFLICT("tripId") DO UPDATE SET "locationName"=EXCLUDED."locationName","latitude"=EXCLUDED."latitude","longitude"=EXCLUDED."longitude","updatedAt"=NOW() RETURNING *`;
    await this.db.$executeRaw`UPDATE "TripWeatherReview" SET "status"='PENDING',"reviewedByAccountId"=NULL,"reviewedAt"=NULL,"notes"='Trip location changed; weather review must be repeated.',"updatedAt"=NOW() WHERE "tripId"=${tripId} AND "status"='APPROVED'`;
  ...
  async latest(tripId:string){const rows=await this.db.$queryRaw<TripWeatherReview[]>`SELECT * FROM "TripWeatherReview" WHERE "tripId"=${tripId} ORDER BY "fetchedAt" DESC,"createdAt" DESC LIMIT 1`;return rows[0]??null;}
  ...
  async refresh(actorAccountId:string,tripId:string){
    ...
    if(previous&&previous.snapshotHash===snapshotHash){
      const rows=await this.db.$queryRaw<TripWeatherReview[]>`UPDATE "TripWeatherReview" SET "provider"=${snapshot.provider??'STORMGLASS'},"forecastAt"=${forecastAt},"fetchedAt"=NOW(),"snapshot"=${JSON.stringify(snapshot)}::jsonb,"updatedAt"=NOW() WHERE "id"=${previous.id} RETURNING *`;review=rows[0];
    }else{
      const rows=await this.db.$queryRaw<TripWeatherReview[]>`INSERT INTO "TripWeatherReview"("tripId","provider","forecastAt","fetchedAt","snapshot","snapshotHash","status","createdAt","updatedAt") VALUES(${tripId},${snapshot.provider??'STORMGLASS'},${forecastAt},NOW(),${JSON.stringify(snapshot)}::jsonb,${snapshotHash},'PENDING',NOW(),NOW()) RETURNING *`;review=rows[0];
    }
  ...
  async decide(actorAccountId:string,tripId:string,input:{status?:TripWeatherReviewStatus;notes?:string|null}){
    ...
    const rows=await this.db.$queryRaw<TripWeatherReview[]>`UPDATE "TripWeatherReview" SET "status"=${status},"notes"=${notes},"reviewedByAccountId"=${actorAccountId},"reviewedAt"=NOW(),"updatedAt"=NOW() WHERE "id"=${current.id} RETURNING *`;
