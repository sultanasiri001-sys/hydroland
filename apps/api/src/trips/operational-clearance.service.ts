...
export class OperationalClearanceService {
  ...
  private async latestEvent(tripId:string){
    return this.db.auditEvent.findFirst({
      where:{resource:'Trip',resourceId:tripId,action:{in:['OPERATIONAL_CLEARANCE_GRANTED','OPERATIONAL_REVIEW_APPROVED','OPERATIONAL_CLEARANCE_REVOKED']}},
      orderBy:{occurredAt:'desc'},
      select:{id:true,action:true,occurredAt:true},
    }) as Promise<ClearanceEvent|null>;
  }
  ...
  async grant(reviewerAccountId:string,tripId:string,reason?:string){
    ...
    const event=await this.audit.record({action,resource:'Trip',resourceId:tripId,metadata:{reviewerAccountId,readiness,reason:reason?.trim()||null}});
    return {tripId,clearance:'GRANTED',readiness,auditEventId:event.id};
  }

  async revokeIfStale(tripId:string){
    const latest=await this.latestEvent(tripId);
    if(!latest||latest.action==='OPERATIONAL_CLEARANCE_REVOKED')return {revoked:false,reason:'NO_ACTIVE_CLEARANCE'};
    const changedAt=await this.latestOperationalChange(tripId);
    if(!changedAt||changedAt<=latest.occurredAt)return {revoked:false,reason:'CURRENT'};
    const event=await this.audit.record({action:'OPERATIONAL_CLEARANCE_REVOKED',resource:'Trip',resourceId:tripId,metadata:{previousClearanceEventId:latest.id,reason:'OPERATIONAL_STATE_CHANGED',changedAt}});
    return {revoked:true,auditEventId:event.id,previousClearanceEventId:latest.id,changedAt};
  }

  async assertValid(tripId:string){
    await this.revokeIfStale(tripId);
    const latest=await this.latestEvent(tripId);
    if(!latest||latest.action==='OPERATIONAL_CLEARANCE_REVOKED')throw new ConflictException('A current operational clearance is required.');
    const readiness=await this.calendar.readinessForTrip(tripId);
    if(readiness.status==='NOT_READY')throw new ConflictException(`Trip is NOT_READY: ${readiness.blockers.join(', ')}`);
    if(readiness.status==='REVIEW_REQUIRED'&&latest.action!=='OPERATIONAL_REVIEW_APPROVED')throw new ConflictException('Current readiness requires documented administrative review approval.');
    return {clearance:latest,readiness};
  }
}