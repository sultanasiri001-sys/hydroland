...
export class AuthService {
  ...
  async register(input:Credentials){
    ...
    await this.db.$transaction(async(tx:Prisma.TransactionClient)=>{
      const account=await tx.account.create({data:{email,passwordHash:this.hash(input.password),person:{create:{firstName:'Pending',lastName:'Profile'}}}});
      await this.replaceChallenge(tx,account.id,'VERIFY_EMAIL',VERIFY_TTL_MS);
    });
  ...
  async login(input:Credentials){
    ...
    if(!account||!this.verify(input.password,account.passwordHash)||!this.isActive(account.status,account.emailVerifiedAt))throw new UnauthorizedException('Invalid credentials.');
  ...
  async refresh(refreshToken:string):Promise<Tokens>{
    ...
    if(!session||session.revokedAt||session.expiresAt<=new Date()||!this.isActive(session.account.status,session.account.emailVerifiedAt))throw new UnauthorizedException('Invalid session.');
  ...
  private access(id:string,sessionId:string){
    ...
    const body=`${encode({alg:'HS256',typ:'JWT'})}.${encode({sub:id,sid:sessionId,typ:'access',iat:now,exp:now+900})}`;
  ...
  private verify(value:string,encoded:string){if(typeof value!=='string'||!value)return false;const [salt,digest]=encoded.split(':');if(!salt||!digest)return false;const expected=Buffer.from(digest,'hex');const actual=scryptSync(value,salt,64);return expected.length===actual.length&&timingSafeEqual(actual,expected);}
