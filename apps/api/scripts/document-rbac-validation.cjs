...
for(const token of ["VIEWER:new Set(['TEMPLATE_LIST','DOCUMENT_READ','BRAND_READ'])","STAFF:new Set(['TEMPLATE_LIST','DOCUMENT_CREATE','DOCUMENT_READ','REVISE','SUBMIT'])","ADMIN:new Set(['TEMPLATE_CREATE'","OWNER:new Set(['TEMPLATE_CREATE'","status!=='ACTIVE'"]) if(!p.includes(token)) throw new Error('Missing RBAC policy: '+token);
