...
const requiredService=[
  ...
  "type:'AUTH_EMAIL_VERIFICATION',status:'PENDING'},data:{status:'FAILED'}",
...
for(const marker of requiredService) assert.ok(service.includes(marker),`Missing session/registration/recovery/Google invariant: ${marker}`);
...
const requiredController=[
  ...
  'const MAX_LOGIN_FAILURES=10','const MAX_REGISTRATION_ATTEMPTS=5','const MAX_PUBLIC_ACTIONS=5','const MAX_MFA_FAILURES=5','const MAX_GOOGLE_FAILURES=10','const MAX_RATE_BUCKETS=5000',
...
for(const marker of requiredController) assert.ok(controller.includes(marker),`Missing auth abuse/recovery/Google/MFA invariant: ${marker}`);
