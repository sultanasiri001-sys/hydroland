...
try{
 ...
 await db.policyControl.updateMany({where:{category:'HR_COMPLIANCE'},data:{state:'DISABLED'}});
 const compAuditBefore=await db.auditEvent.count({where:{resource:'CompensationTerm',resourceId:compensation.id,action:'HR_COMPENSATION_APPROVED'}});
 r=await compPatch(); if(r.status!==409)throw new Error('Expected compensation compliance denial 409, got '+r.status+' '+await r.text());
 const deniedComp=await db.compensationTerm.findUniqueOrThrow({where:{id:compensation.id}}); if(deniedComp.status!=='APPROVAL_REQUIRED'||deniedComp.approvedByAccountId)throw new Error('Compensation compliance denial mutated state');
 if(await db.auditEvent.count({where:{resource:'CompensationTerm',resourceId:compensation.id,action:'HR_COMPENSATION_APPROVED'}})!==compAuditBefore)throw new Error('Compensation compliance denial emitted success audit');
 await db.policyControl.updateMany({where:{category:'HR_COMPLIANCE'},data:{state:'ENABLED'}});
 ...
 await db.policyControl.updateMany({where:{category:'HR_COMPLIANCE'},data:{state:'DISABLED'}});
 const relAuditBefore=await db.auditEvent.count({where:{resource:'EmployeeRelationsCase',resourceId:relations.id,action:'HR_DISCIPLINARY_DECISION_APPROVED'}});
 r=await relPatch(); if(r.status!==409)throw new Error('Expected disciplinary compliance denial 409, got '+r.status+' '+await r.text());
 const deniedRel=await db.employeeRelationsCase.findUniqueOrThrow({where:{id:relations.id}}); if(deniedRel.status!=='APPROVAL_REQUIRED'||deniedRel.approvedByAccountId)throw new Error('Disciplinary compliance denial mutated state');
 if(await db.auditEvent.count({where:{resource:'EmployeeRelationsCase',resourceId:relations.id,action:'HR_DISCIPLINARY_DECISION_APPROVED'}})!==relAuditBefore)throw new Error('Disciplinary compliance denial emitted success audit');
 await db.policyControl.updateMany({where:{category:'HR_COMPLIANCE'},data:{state:'ENABLED'}});
