...
const account=await db.account.create({data:{personId:person.id,email:`hr-e2e-${suffix}@example.invalid`,passwordHash:'e2e',nextStatus:'ACTIVE',emailVerifiedAt:new Date()}});
...
await db.organizationMember.create({data:{organizationId:org.id,accountId:account.id,role:'STAFF',nextStatus:'ACTIVE'}});
await db.roleAssignment.create({data:{accountId:account.id,role:'HR_MANAGER',nextStatus:'ACTIVE',scope:{organizationIds:[org.id]}}});
