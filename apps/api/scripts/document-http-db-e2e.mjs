...
const org=await db.organization.create({data:{displayName:'Document E2E '+suffix,kind:'DIVE_CENTER',ownerId:signer.id,documentBrandNameAr:'مركز اختبار الغوص',documentBrandNameEn:'E2E Dive Center',documentBrandVersion:1}});
...
try{
 ...
 const body={organizationId:org.id,templateId:template.id,department:templateBody.department,contentHash:'sha256:'+suffix,payload:{summary:'اختبار عربي HYD-2026 1,234.50'}};
 ...
 if(printContract.branding?.brandVersion!==1||printContract.branding?.brandNameAr!=='مركز اختبار الغوص'||printContract.branding?.brandNameEn!=='E2E Dive Center')throw new Error('Pinned print branding invalid');
 const initialSnapshot=await db.documentBrandSnapshot.findUnique({where:{organizationId_brandVersion:{organizationId:org.id,brandVersion:1}}});if(!initialSnapshot||initialSnapshot.brandNameAr!=='مركز اختبار الغوص'||initialSnapshot.brandNameEn!=='E2E Dive Center')throw new Error('Initial branding snapshot was not created automatically');
 r=await call(`/documents/organizations/${org.id}/branding`,'PUT',token(approver.id),{brandNameAr:'مركز اختبار الغوص الإصدار الثاني',brandNameEn:'E2E Dive Center v2',footerAr:'تشغيل بواسطة هايدرولاند',footerEn:'Current branding v2'});if(!r.ok)throw new Error('Branding update failed '+r.status+' '+await r.text());const brandV2=await r.json();if(brandV2.brandVersion!==2)throw new Error('Branding version did not advance to v2');
 r=await call(`/documents/${doc.id}/print-contract`,'GET',token(viewer.id));if(!r.ok)throw new Error('Pinned print contract reread failed');const pinnedAfterBrandChange=await r.json();if(pinnedAfterBrandChange.branding?.brandVersion!==1||pinnedAfterBrandChange.branding?.brandNameAr!=='مركز اختبار الغوص'||pinnedAfterBrandChange.branding?.brandNameEn!=='E2E Dive Center')throw new Error('Historical document branding changed after organization branding update');
 ...
 r=await call(`/documents/${doc.id}/pdf`,'GET',token(viewer.id));if(!r.ok)throw new Error('VIEWER PDF read failed '+r.status+' '+await r.text());if(r.headers.get('content-type')!=='application/pdf')throw new Error('PDF content type invalid');const pdfBytes=new Uint8Array(await r.arrayBuffer());if(new TextDecoder().decode(pdfBytes.slice(0,5))!=='%PDF-')throw new Error('PDF signature invalid');const parsedPdf=await PDFDocument.load(pdfBytes);if(parsedPdf.getPageCount()<1)throw new Error('PDF page count invalid');const pdfObjects=Array.from(parsedPdf.context.enumerateIndirectObjects(),([,object])=>String(object)).join('\n');if(!/NotoSansArabic/i.test(pdfObjects))throw new Error('Arabic PDF font is not embedded');
 ...
 const printSummary=printContract.fields?.find(x=>x.key==='summary');if(!printSummary||printSummary.value!=='اختبار عربي HYD-2026 1,234.50'||printSummary.labelAr!=='الملخص'||printSummary.labelEn!=='Summary'||printSummary.required!==true)throw new Error('Print contract field mapping invalid');
 ...
 if(snapshots.length!==2||snapshots[0].version!==1||snapshots[0].payload?.summary!=='اختبار عربي HYD-2026 1,234.50'||snapshots[1].version!==2||snapshots[1].payload?.summary!=='revision 2')throw new Error('Immutable revision snapshots invalid '+JSON.stringify(snapshots.map(x=>({version:x.version,payload:x.payload}))));
 ...
 r=await call(`/documents/${doc.id}/revisions/1`,'GET',token(viewer.id));if(!r.ok||(await r.json()).payload?.summary!=='اختبار عربي HYD-2026 1,234.50')throw new Error('Revision v1 API invalid');
