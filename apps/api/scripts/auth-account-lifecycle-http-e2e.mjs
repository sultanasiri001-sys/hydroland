...
try{
  let result=await request('/auth/register',{body:{email,password:oldPassword}});
  assert(result.response.status===201,`register expected 201, got ${result.response.status}: ${JSON.stringify(result.payload)}`);
  assert(result.payload.verificationRequired===true,'registration must require verification');
  ...
  result=await request('/auth/logout',{body:{refreshToken:verifiedRefresh}});
  assert(result.response.status===204,`logout expected 204, got ${result.response.status}`);
  result=await request('/me',{method:'GET',headers:{authorization:`Bearer ${verifiedAccess}`}});
  assert(result.response.status===401,`logged-out access token must be revoked immediately, got ${result.response.status}`);

  result=await request('/auth/email-verification/confirm',{body:{token:verifyToken}});
  assert(result.response.status===401,`verification token replay expected 401, got ${result.response.status}`);

  result=await request('/auth/login',{body:{email,password:oldPassword}});
  assert(result.response.ok,`verified login failed: ${result.response.status}`);
  const preResetAccess=result.payload.accessToken,preResetRefresh=result.payload.refreshToken;

  result=await request('/auth/password-reset/request',{body:{email}});
  ...
  result=await request('/me',{method:'GET',headers:{authorization:`Bearer ${preResetAccess}`}});
  assert(result.response.status===401,`password reset must revoke existing access sessions, got ${result.response.status}`);
  result=await request('/auth/refresh',{body:{refreshToken:preResetRefresh}});
  ...
  result=await request('/auth/password-reset/request',{body:{email:`missing-${suffix}@example.invalid`}});
  ...
  console.log('Auth account lifecycle HTTP/DB E2E passed: pending registration, one-time verification, session-bound access revocation, one-time reset, full session revocation, and enumeration-safe reset requests.');
} finally {
  if(accountId){
