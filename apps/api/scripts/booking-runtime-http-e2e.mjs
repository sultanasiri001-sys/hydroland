...
try{
  ...
  if(trip){await db.crewAssignment.deleteMany({where:{tripId:trip.id}}).catch(()=>{});await db.bookingParticipant.deleteMany({where:{booking:{tripId:trip.id}}}).catch(()=>{});await db.payment.deleteMany({where:{booking:{tripId:trip.id}}}).catch(()=>{});await db.booking.deleteMany({where:{tripId:trip.id}}).catch(()=>{});await db.safetyChecklist.deleteMany({where:{tripId:trip.id}}).catch(()=>{});await db.$executeRaw`DELETE FROM "TripWeatherReview" WHERE "tripId"=${trip.id}`.catch(()=>{});await db.$executeRaw`DELETE FROM "TripOperationalLocation" WHERE "tripId"=${trip.id}`.catch(()=>{});}
