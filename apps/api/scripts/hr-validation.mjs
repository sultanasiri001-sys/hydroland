...
const requiredMigrationEntities = [
  'OrgUnit','Position','Employment','EmploymentContract','EmploymentMovement',
...
for (const token of ['Maker','Reviewer','Approver','IAM','Audit','external centers']) {
  if (!l4.toLowerCase().includes(token.toLowerCase()) && !l3.toLowerCase().includes(token.toLowerCase())) {
    throw new Error(`Missing HR governance control: ${token}`);
  }
}
...
console.log('HR L1-L4 architecture, migration and Prisma reconciliation validated.');
