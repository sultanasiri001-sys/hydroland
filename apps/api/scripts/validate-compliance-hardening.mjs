...
const checks = [
  ...
  ['PASS maps to ALLOWED', /if\s*\(complianceDecision\s*===\s*['"]PASS['"]\)\s*return\s*['"]ALLOWED['"]/.test(service)],
  ['REVIEW maps to REVIEW_REQUIRED', /if\s*\(complianceDecision\s*===\s*['"]REVIEW['"]\)\s*return\s*['"]REVIEW_REQUIRED['"]/.test(service)],
  ['BLOCK and ESCALATE fall through to DEFERRED', /if\s*\(complianceDecision\s*===\s*['"]REVIEW['"]\)\s*return\s*['"]REVIEW_REQUIRED['"];\s*return\s*['"]DEFERRED['"]/.test(service)],
