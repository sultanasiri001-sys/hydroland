...
export async function getAccessToken(){const current=await loadSession();if(!current)return null;return current.accessToken||((await refreshSession())?.accessToken??null)}
