...
function accessIsUsable(token:string){try{const part=token.split('.')[1];if(!part)return false;const normalized=part.replace(/-/g,'+').replace(/_/g,'/');const payload=JSON.parse(globalThis.atob(normalized));return typeof payload.exp==='number'&&payload.exp*1000>Date.now()+30000}catch{return false}}
...
export async function getAccessToken(){const current=await loadSession();if(!current)return null;if(accessIsUsable(current.accessToken))return current.accessToken;return (await refreshSession())?.accessToken??null}
